...
namespace procesos_app.Controllers.API
{
    public class PublicarController : ApiController
    {
        ...
        public HttpResponseMessage Publicar(PublicarDTO @actual)
        {
            try
            {
                var curr = _context.StudentSection.Where(x => x.SectionId == @actual.SectionId && x.StudentId == @actual.StudentId).FirstOrDefault();

                curr.FinalScore = @actual.FinalScore;
                if(@actual.FinalScore > 70)
                {
                    curr.Status = procesos_app.Models.Enums.StatusSectionEnum.SectionStatus.Satisfactorio;
                }
                else
                {
                    curr.Status = procesos_app.Models.Enums.StatusSectionEnum.SectionStatus.InSatisfactorio;
                }
